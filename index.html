...
<head>
    ...
    <meta name="description" content="AdMadia — a modern affiliate network offering clean design, 24/7 support, custom campaign controls, powerful performance, weekly payments and advanced statistics.">
    <title>AdMadia — Services That We Provide | Affiliate Network</title>

    <!-- ============================================================
         SECURITY HEADERS (meta-level)
         ------------------------------------------------------------
         NOTE: Content-Security-Policy delivered via <meta> cannot use
         frame-ancestors / report-uri / sandbox. For maximum protection
         also send real HTTP headers from your host (Netlify _headers,
         Cloudflare rules, nginx add_header, etc.):
    ...
    <meta http-equiv="Content-Security-Policy"
          content="default-src 'self'; base-uri 'self'; object-src 'none'; form-action 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval' https://www.gstatic.com https://cdn.jsdelivr.net https://*.googleapis.com; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src 'self' data: https://fonts.gstatic.com; img-src 'self' data: blob: https://i.pravatar.cc https://*.googleusercontent.com; media-src 'self' data:; connect-src 'self' https://*.googleapis.com https://*.firebaseio.com https://*.firebasedatabase.app wss://*.firebaseio.com wss://*.firebasedatabase.app https://api.ipify.org; worker-src 'self' blob:;">
    ...
    <!-- Charts (version-pinned; jQuery removed — it was unused except for 3 lines and carried known CVEs) -->
    <script src="https://cdn.jsdelivr.net/npm/chart.js@4.4.1/dist/chart.umd.min.js"></script>

    <style>
    /* ============================================================
       PART A — LANDING PAGE STYLES (public marketing homepage)
       ============================================================ */
    :root{
      --brand-blue:      #3699ff;
      --brand-blue-dark: #1a7fe0;
      --brand-red:       #f64e60;
      --lp-bg:           #ffffff;
      --lp-bg-soft:      #f4f6f9;
      --lp-text:         #3f4254;
      ...
      --lp-border:       #e4e6ef;
      --lp-radius:       12px;
      --lp-shadow-sm:    0 2px 10px rgba(63,66,84,.05);
      --lp-shadow-lg:    0 14px 34px rgba(54,153,255,.14);
      --header-h:        72px;
      --ease:            cubic-bezier(.22,.61,.36,1);
    }

    /* Landing container — fullscreen scrollable when visible */
    #landing-view{
      position:fixed;
      inset:0;
      z-index:600;
    ...
    #landing-view *,
    #landing-view *::before,
    #landing-view *::after{ box-sizing:border-box; }

    #landing-view img,
    #landing-view svg{ display:block; max-width:100%; }
    #landing-view a{ text-decoration:none; color:inherit; }
    #landing-view ul{ list-style:none; margin:0; padding:0; }
    #landing-view h1,
    #landing-view h2,
    #landing-view h3,
    ...
    .site-header.scrolled{
      ...
      box-shadow:0 4px 22px rgba(63,66,84,.07);
    }
    .header-inner{
      display:flex;
      align-items:center;
    ...
    .brand-mark{
      ...
      box-shadow:0 0 16px rgba(54,153,255,.35);
      color:#fff; font-size:18px; line-height:1;
    }
    .brand-name .ad{ color:var(--brand-blue); }
    .brand-name .madia{ color:var(--brand-red); }
    ...
    .nav-toggle:hover{ border-color:var(--brand-blue); }
    .nav-toggle span{
      display:block;
    ...
    .nav-toggle.open span::before{ top:0; transform:rotate(45deg); background:var(--brand-blue); }
    .nav-toggle.open span::after { top:0; transform:rotate(-45deg); background:var(--brand-blue); }
    ...
    .nav-links a:hover{
      ...
      background:rgba(54,153,255,.08);
      transform:translateY(-1px);
    }
    .nav-links a.nav-login{ border-color:var(--lp-border); }
    .nav-links a.nav-login:hover{ border-color:var(--brand-blue); }
    .nav-links a.nav-register{
      background:var(--brand-blue);
      color:#fff;
      padding-inline:20px;
      box-shadow:0 6px 16px rgba(54,153,255,.28);
    }
    .nav-links a.nav-register:hover{
      background:var(--brand-blue-dark);
      color:#fff;
      box-shadow:0 9px 22px rgba(54,153,255,.38);
    }
    ...
    .hero{
      ...
      padding:78px 0 34px;
      background:
        radial-gradient(900px 380px at 50% -140px, rgba(54,153,255,.13), transparent 70%),
        linear-gradient(180deg,var(--lp-bg-soft) 0%, #ffffff 100%);
    }
    .hero-eyebrow{
      display:inline-flex;
      align-items:center;
      ...
      color:var(--brand-blue);
      background:rgba(54,153,255,.09);
      border:1px solid rgba(54,153,255,.22);
      padding:7px 16px;
      border-radius:999px;
      margin-bottom:22px;
    }
    .hero-eyebrow .dot{
      ...
      border-radius:50%;
      background:var(--brand-blue);
      box-shadow:0 0 0 4px rgba(54,153,255,.18);
    }
    .hero h1{
    ...
    /* ---- Services grid (3 rows × 2 columns = 6 cards) ---- */
    .services{ padding:16px 0 100px; }
    .services-grid{
      display:grid;
    ...
    .service-icon{
      ...
      color:var(--brand-blue);
      background:rgba(54,153,255,.08);
      border:1px solid rgba(54,153,255,.14);
    ...
    .footer-links a{ transition:color .25s var(--ease); cursor:pointer; }
    ...
    @media (max-width:880px){
      ...
      .nav-links{
        ...
        box-shadow:0 16px 34px rgba(63,66,84,.10);
        opacity:0;
        visibility:hidden;
      ...
      .nav-links a{ text-align:center; padding:13px 16px; font-size:13.5px; }
      .nav-links a.nav-login,
      .nav-links a.nav-register{ padding-inline:16px; }
    }
    @media (max-width:768px){
      ...
      .service-icon{ flex-basis:56px; width:56px; height:56px; border-radius:14px; }
      .service-icon svg{ width:28px; height:28px; }
    }
    @media (max-width:420px){
      .lp-container{ padding-inline:18px; }
    ...
    /* ============================================================
       PART B — AUTH + DASHBOARD STYLES (from original AdMadia app)
       ============================================================ */
    :root {
        --bg-dark: #f4f6f9;
        --sidebar-bg: #ffffff;
    ...
    .notification-list { list-style: none; max-height: 250px; overflow-y: auto; padding: 0; margin: 0; }
    ...
    .active-affiliates-list { list-style: none; max-height: 320px; overflow-y: auto; padding-right: 5px; padding-left: 0; margin: 0; }
...
<div id="landing-view">
  <header class="site-header" id="siteHeader">
    <div class="lp-container header-inner">
      <a href="#home" class="brand" id="brandHome" aria-label="AdMadia home">
        <span class="brand-mark" aria-hidden="true">⚡</span>
        <span class="brand-name"><span class="ad">Ad</span><span class="madia">Madia</span></span>
      </a>
      ...
      <nav aria-label="Main navigation">
        ...
        <ul class="nav-links" id="navLinks">
          ...
          <li><a id="navLogin" class="nav-login">LOGIN</a></li>
          <li><a id="navRegister" class="nav-register">REGISTER</a></li>
        </ul>
      </nav>
    </div>
  </header>
  ...
  <section class="hero" id="home">
    <div class="lp-container">
      <span class="hero-eyebrow"><span class="dot"></span>AdMadia Network</span>
      <h1>Services that we <span class="accent">provide</span></h1>
      <p>Everything you need to run, scale and monetise your affiliate traffic — built into one fast, reliable and beautifully simple platform.</p>
    </div>
  </section>
    ...
    <div class="lp-container">
      <div class="services-grid">
        ...
        <article class="service-card reveal">
          <div class="service-icon" aria-hidden="true">
            <svg viewBox="0 0 48 48">
              ...
              <path d="M17 16v16"/>
              <path d="M4 36h40"/>
              <path d="M20 36l1.2-4h5.6L28 36"/>
            </svg>
          </div>
          <div class="service-body">
            <h3>Clean Design</h3>
            <p>Our clean dashboard design is built with you in mind. Free of clutter and easy to navigate.</p>
          </div>
        </article>
        ...
        <article class="service-card reveal">
          <div class="service-icon" aria-hidden="true">
            <svg viewBox="0 0 48 48">
              <path d="M10 30v-6a14 14 0 0 1 28 0v6"/>
              <rect x="6" y="28" width="9" height="13" rx="4.5"/>
              <rect x="33" y="28" width="9" height="13" rx="4.5"/>
            </svg>
          </div>
          <div class="service-body">
            <h3>24/7 Customer Support</h3>
            <p>Our customer support team are available to help you at all hours of the day, 7 days a week.</p>
          </div>
        </article>
        ...
        <article class="service-card reveal">
          <div class="service-icon" aria-hidden="true">
            <svg viewBox="0 0 48 48">
              <rect x="5" y="10" width="27" height="13" rx="6.5"/>
              <circle cx="25.5" cy="16.5" r="4"/>
              <rect x="16" y="26" width="27" height="13" rx="6.5"/>
              <circle cx="22.5" cy="32.5" r="4"/>
            </svg>
          </div>
          <div class="service-body">
            <h3>Custom Campaign Controls</h3>
            <p>Custom controls that allow you to boost or pause any campaign at the single click of a button.</p>
          </div>
        </article>
        ...
        <article class="service-card reveal">
          <div class="service-icon" aria-hidden="true">
            <svg viewBox="0 0 48 48">
              <path d="M24 5 39 11v11c0 9.6-6.2 16.6-15 21-8.8-4.4-15-11.4-15-21V11z"/>
              <path d="M26 14 17 26h6l-1.5 9L30 23h-6z"/>
            </svg>
          </div>
          <div class="service-body">
            <h3>Powerful Performance</h3>
            <p>Lightning fast servers powering our dashboard, ensuring optimal performance and high uptime.</p>
          </div>
        </article>
        ...
        <article class="service-card reveal">
          <div class="service-icon" aria-hidden="true">
            <svg viewBox="0 0 48 48">
              <path d="M9.5 27.5A14 14 0 0 1 33 13.5"/>
              <polyline points="28.5 10.5 34.8 13.8 31.5 19.8"/>
              <path d="M38.5 20.5A14 14 0 0 1 15 34.5"/>
              <polyline points="19.5 37.5 13.2 34.2 16.5 28.2"/>
              <path d="M24 18.6v10.8"/>
              <path d="M27 21.2c0-1.5-1.3-2.4-3-2.4s-3 .9-3 2.4 1.3 2.4 3 2.4 3 .9 3 2.4-1.3 2.4-3 2.4-3-.9-3-2.4"/>
            </svg>
          </div>
          <div class="service-body">
            <h3>Weekly Payments</h3>
            <p>Weekly payments with a low minimum withdrawal threshold, you can reinvest your income when it suits you.</p>
          </div>
        </article>
        ...
        <article class="service-card reveal">
          <div class="service-icon" aria-hidden="true">
            <svg viewBox="0 0 48 48">
              <path d="M8 6v30a2 2 0 0 0 2 2h30"/>
              <rect x="15" y="26" width="6" height="12" rx="1.5"/>
              <rect x="24" y="20" width="6" height="18" rx="1.5"/>
              <rect x="33" y="14" width="6" height="24" rx="1.5"/>
            </svg>
          </div>
          <div class="service-body">
            <h3>Advanced Statistics</h3>
            <p>Our advanced statistics will allow you to breakdown your traffic reports to the smallest of details.</p>
          </div>
        </article>
  ...
  <footer class="site-footer" id="contact">
    <div class="lp-container footer-inner">
      <span>© <span id="year">2026</span> AdMadia Network. All rights reserved.</span>
      <nav class="footer-links" aria-label="Footer navigation">
        <a id="footHome">Home</a>
        <a id="footContact">Contact Us</a>
        <a id="footLogin">Login</a>
        <a id="footRegister">Register</a>
      </nav>
    </div>
  </footer>
</div>
...
<!-- ============================================================
     AUTH SECTION — Login / Signup / Forgot Password
     ============================================================ -->
<div id="auth-wrapper" class="auth-wrapper hidden">
    <!-- LOGIN -->
    <div id="login-section" class="auth-container">
        ...
        <div class="form-group">
            ...
            <input type="email" id="login-email" class="form-control" placeholder="user@domain.com" autocomplete="username" maxlength="254">
        </div>
        <div class="form-group">
            <label>Password</label>
            <input type="password" id="login-password" class="form-control" placeholder="••••••••" autocomplete="current-password" maxlength="128">
        </div>
        ...
        <div class="toggle-auth" style="margin-top: 10px;">
            <span id="back-to-landing">← Back to Home</span>
        </div>
    </div>
    ...
    <div id="forgot-section" class="auth-container hidden">
        ...
        <div class="form-group">
            <label>Email Address</label>
            <input type="email" id="forgot-email" class="form-control" placeholder="user@domain.com" autocomplete="username" maxlength="254">
        </div>
    ...
    <div id="signup-section" class="auth-container wide hidden">
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <input type="text" id="signup-firstname" class="form-control" placeholder="John" maxlength="60" autocomplete="given-name">
            </div>
            <div class="form-group">
                <label>Last Name <span class="req">*</span></label>
                <input type="text" id="signup-lastname" class="form-control" placeholder="Doe" maxlength="60" autocomplete="family-name">
            </div>
        </div>
        <div class="form-row">
            <div class="form-group">
                <label>Username <span class="req">*</span></label>
                <input type="text" id="signup-username" class="form-control" placeholder="johndoe123" maxlength="40" autocomplete="off">
            </div>
            <div class="form-group">
                <label>Affiliate FF ID <span class="req">*</span></label>
                <input type="text" id="signup-ffid" class="form-control" placeholder="FF-00000 (your unique ID)" maxlength="40" autocomplete="off">
            </div>
        </div>
        <div class="form-row">
            <div class="form-group">
                <label>Password <span class="req">*</span></label>
                <input type="password" id="signup-password" class="form-control" placeholder="••••••••" autocomplete="new-password" maxlength="128">
            </div>
            <div class="form-group">
                <label>Re-enter Password <span class="req">*</span></label>
                <input type="password" id="signup-password2" class="form-control" placeholder="••••••••" autocomplete="new-password" maxlength="128">
            </div>
        </div>
        <div class="form-group">
            <label>Email Address <span class="req">*</span></label>
            <input type="email" id="signup-email" class="form-control" placeholder="user@domain.com" autocomplete="email" maxlength="254">
        </div>
        <div class="form-row">
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <input type="text" id="signup-promo" class="form-control" placeholder="Enter or type NONE" maxlength="40">
            </div>
        </div>
        <div class="form-group">
            <label>Street Address <span class="req">*</span></label>
            <input type="text" id="signup-street" class="form-control" placeholder="123 Main Street" maxlength="160" autocomplete="street-address">
        </div>
        <div class="form-row">
            <div class="form-group">
                <label>State <span class="req">*</span></label>
                <input type="text" id="signup-state" class="form-control" placeholder="State / Province" maxlength="80">
            </div>
            <div class="form-group">
                <label>City <span class="req">*</span></label>
                <input type="text" id="signup-city" class="form-control" placeholder="City" maxlength="80">
            </div>
        </div>
        <div class="form-row">
            <div class="form-group">
                <label>Zip / Postal Code <span class="req">*</span></label>
                <input type="text" id="signup-zip" class="form-control" placeholder="00000" maxlength="20" autocomplete="postal-code">
            </div>
            <div class="form-group">
                <label>Where did you hear about us? <span class="req">*</span></label>
                <input type="text" id="signup-hear" class="form-control" placeholder="Google, Friend, Forum..." maxlength="120">
            </div>
        </div>
        <div class="form-group">
            <label>Website(s)</label>
            <input type="text" id="signup-websites" class="form-control" placeholder="https://yoursite.com (optional)" maxlength="240">
        </div>
        <div class="form-row">
            <div class="form-group">
                <label>Promotional Method / Traffic Sources <span class="req">*</span></label>
                <input type="text" id="signup-traffic" class="form-control" placeholder="SEO, Social, Email..." maxlength="160">
            </div>
            <div class="form-group">
                <label>Current Networks</label>
                <input type="text" id="signup-networks" class="form-control" placeholder="e.g. AdCombo, MaxBounty..." maxlength="160">
            </div>
        </div>
        ...
        <div class="toggle-auth" style="margin-top: 8px;">
            <span id="back-to-landing-2">← Back to Home</span>
        </div>
    </div>
</div>
    ...
    <div class="main-wrapper" style="margin-top: 78px;">
        ...
        <div id="admin-view-overview" class="hidden">
            <div class="box-container" style="margin-bottom: 20px;">
                ...
                <div class="metrics-grid-2x2" style="margin-bottom: 0;">
                    <div class="metric-box box-clicks"><h2 id="ov-clicks">0</h2><h4>Total Clicks</h4><div class="trend-indicator">All Affiliates</div></div>
                    <div class="metric-box box-leads"><h2 id="ov-conversions">0</h2><h4>Conversions</h4><div class="trend-indicator" id="ov-cr">0.00% CR</div></div>
                    <div class="metric-box box-earn"><h2 id="ov-revenue">$0.00</h2><h4>Revenue</h4><div class="trend-indicator" id="ov-epc">$0.00 EPC</div></div>
                    <div class="metric-box box-epc"><h2 id="ov-payouts">$0.00</h2><h4>Payouts</h4><div class="trend-indicator">Affiliate Share</div></div>
                </div>
            </div>
            ...
            <div class="ov-chart-panel" style="margin-bottom: 25px;">
                ...
                <div class="ov-chart-wrap"><canvas id="adminOverviewChart"></canvas></div>
            ...
            <div class="box-container">
                <div class="box-header"><span>⚡ Quick Admin Actions</span></div>
        ...
        <div id="admin-view-redirect" class="box-container hidden">
            ...
            <div class="settings-grid-2">
                <div class="redirect-card">
                    ...
                    <div class="redirect-toggle-row">
                        <div>
                            ...
                            <div class="redirect-sub">When ON, every affiliate tracking link generated by the network is routed to the Target URL below instead of the default offer destination.</div>
                        </div>
                ...
                <div class="redirect-card">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="redirect-target-url" class="form-control" placeholder="https://example.com/target-offer" style="background: var(--card-bg);" maxlength="2000">
                    </div>
                    <button class="btn-submit" id="btn-save-redirect" style="width: 220px; background:#1bc5bd;">Save Redirect Settings</button>
                    <div style="margin-top:16px;">
                        <div style="font-size:11.5px; color:var(--text-muted); margin-bottom:6px; font-weight:600;">Auto-appended tracking tokens:</div>
            ...
            <div class="preview-box">
                ...
                <div class="preview-desc">This is exactly how a generated affiliate tracking link will look with the current Redirect Controller configuration.</div>
            ...
            <div class="settings-grid-2">
                <div class="redirect-card">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="global-postback-input" class="form-control" placeholder="https://yournetwork.com/postback?clickid={clickid}&payout={payout}" style="background: var(--card-bg);" maxlength="2000">
                    </div>
                    <button class="btn-submit" id="btn-save-global-postback" style="width: 220px; background:#1bc5bd;">Save Global Postback</button>
                    <div style="margin-top:16px;">
                        <div style="font-size:11.5px; color:var(--text-muted); margin-bottom:6px; font-weight:600;">Available tokens:</div>
                        <span class="token-chip">{clickid}</span>
                ...
                <div class="redirect-card">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="custom-postback-email" class="form-control" placeholder="affiliate@domain.com" style="background: var(--card-bg);" maxlength="254">
                    </div>
                    <div class="form-group">
                        <label>Custom Postback URL</label>
                        <input type="text" id="custom-postback-url" class="form-control" placeholder="https://affiliate.com/pb?cid={clickid}&amt={payout}" style="background: var(--card-bg);" maxlength="2000">
                    </div>
                    <button class="btn-submit" id="btn-save-custom-postback" style="width: 220px; background:#8950fc;">Save Custom Postback</button>
                    <div style="font-size:11.5px; color:var(--text-muted); margin-top:12px; line-height:1.6;">Custom postbacks override the global endpoint for that specific affiliate only.</div>
                </div>
            </div>
            <div class="box-header" style="margin-top: 28px;"><span>📋 Custom Postback Registry</span><span style="font-size:11px; color:#1bc5bd;">Live</span></div>
            <div class="table-wrap">
                <table class="data-table">
                    <thead><tr><th>Affiliate</th><th>Custom Postback URL</th><th>Status</th><th>Last Updated</th><th>Actions</th></tr></thead>
        ...
        <div id="admin-view-invoices" class="box-container hidden">
            <div class="box-header">
                ...
                <button class="btn-submit" id="btn-generate-invoices" style="margin-top:0; width:auto; padding:9px 18px; background:#8950fc;">⚙ Run Monday Invoice Generation</button>
            </div>
            ...
            <div style="display:flex; gap:10px; flex-wrap:wrap; margin-bottom:16px; align-items:center;">
                <input type="text" id="invoice-search" class="form-control" placeholder="Search by affiliate email or invoice ID..." style="max-width:340px; background: var(--bg-dark);" maxlength="120">
            ...
            <div class="table-wrap">
                <table class="data-table">
                    <thead><tr><th>Invoice ID</th><th>Affiliate</th><th>Period</th><th>Amount</th><th>Method</th><th>Created</th><th>Status</th><th>Actions</th></tr></thead>
        ...
        <div id="admin-view-settings" class="box-container hidden">
            ...
            <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 20px; margin-bottom: 20px;">
                <div style="background: var(--bg-dark); padding: 20px; border-radius: 6px; border: 1px solid var(--border-color);">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="admin-api-key-input" class="form-control" value="https://app.trcefy.com/click?pid=2&offer_id=23972&sub2=u298346&sub5=s1" style="background: var(--card-bg);" maxlength="2000">
                    </div>
                    <div class="form-group">
                        <label>Master Global Smart Link</label>
                        <input type="text" id="admin-master-link-input" class="form-control" value="https://app.trcefy.com/click?pid=2&offer_id=23972&sub2=u298346&sub5=s1" style="background: var(--card-bg);" maxlength="2000">
                    </div>
            ...
            <div style="background: var(--bg-dark); padding: 20px; border-radius: 6px; border: 1px solid var(--border-color); margin-bottom: 20px;">
                ...
                <div style="display: flex; gap: 10px; flex-wrap: wrap;">
                    <input type="text" id="admin-click-id-search" class="form-control" placeholder="Enter Unique Click ID" style="flex: 1; min-width: 200px; background: var(--card-bg);" maxlength="120">
            ...
            <div style="background: var(--bg-dark); padding: 20px; border-radius: 6px; border: 1px solid var(--border-color); margin-bottom: 20px;">
                ...
                <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 15px;">
                    <div class="form-group">
                        ...
                        <input type="text" id="admin-target-user" class="form-control" placeholder="user@domain.com" style="background: var(--card-bg);" maxlength="254">
                    </div>
                    <div class="form-group">
                        <label>Amount ($ / Lead)</label>
                        <input type="number" id="admin-target-amount" class="form-control" placeholder="50.00" style="background: var(--card-bg);" step="0.01">
                    </div>
                    <div class="form-group">
            ...
            <!-- PROMO REDEMPTION APPROVALS (new — replaces insecure self-crediting) -->
            <div style="background: var(--bg-dark); padding: 20px; border-radius: 6px; border: 1px solid var(--border-color);">
                <div class="box-header" style="margin-bottom:14px; padding-bottom:10px;">
                    <span style="font-size:15px;">🎁 Promo Code Redemption Requests</span>
                    <span style="font-size:11px; color:#1bc5bd;">Live</span>
                </div>
                <div class="table-wrap">
                    <table class="data-table">
                        <thead><tr><th>Affiliate</th><th>Promo Code</th><th>Reward</th><th>Requested</th><th>Status</th><th>Actions</th></tr></thead>
                        <tbody id="admin-promo-redemptions-body">
                            <tr><td colspan="6" style="text-align:center; color: var(--text-muted); padding: 15px;">No redemption requests.</td></tr>
                        </tbody>
                    </table>
                </div>
            </div>
            ...
            <div style="display:grid; grid-template-columns: 320px 1fr; gap:15px; height:560px; min-height:400px;">
                ...
                <div style="display:flex; flex-direction:column; background:var(--bg-dark); border-radius:6px; border:1px solid var(--border-color); overflow:hidden;">
                    ...
                    <div class="chat-input-row" style="padding:12px 15px; margin:0; border-top:1px solid var(--border-color); background:var(--bg-dark);">
                        <input type="text" id="admin-inbox-input" placeholder="Type a message to the affiliate..." maxlength="2000">
        ...
        <div id="admin-view-offers" class="box-container hidden">
            ...
            <div style="background: var(--bg-dark); padding: 20px; border-radius: 6px; border: 1px solid var(--border-color); margin-bottom: 20px;">
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="text" id="offer-add-name" class="form-control" placeholder="e.g. Crypto Wallet App" style="background: var(--card-bg);" maxlength="160">
                    </div>
                    <div class="form-group">
                        <label>Country (ISO code)</label>
                        <input type="text" id="offer-add-country" class="form-control" placeholder="US" style="background: var(--card-bg);" maxlength="8">
                    </div>
                </div>
                <div class="form-row">
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        <label>Payout</label>
                        <input type="text" id="offer-add-payout" class="form-control" placeholder="$20 - $50" style="background: var(--card-bg);" maxlength="40">
                    </div>
                </div>
                <div class="form-group">
                    <label>Base Tracking URL (must end with sub5=s1)</label>
                    <input type="text" id="offer-add-url" class="form-control" value="https://app.trcefy.com/click?pid=2&offer_id=23972&sub2=u298346&sub5=s1" style="background: var(--card-bg);" maxlength="2000">
                </div>
            ...
            <div class="box-header"><span>All Network Offers</span>
                <input type="text" id="admin-offer-search" class="form-control" placeholder="Search offers..." style="width: 250px; background: var(--bg-dark);" maxlength="120">
            </div>
            <div class="table-wrap">
                <table class="data-table">
                    <thead><tr><th>ID</th><th>Name</th><th>Country</th><th>Category</th><th>Payout</th><th>Actions</th></tr></thead>
        ...
        <div id="admin-view-users" class="box-container hidden">
            <div class="box-header">
                ...
                <input type="text" id="admin-user-search" class="form-control" placeholder="Search by Email or ID..." style="width: 250px; background: var(--bg-dark);" maxlength="120">
            </div>
            <div class="table-wrap">
                <table class="data-table">
                    <thead><tr><th>User ID</th><th>Name</th><th>Email</th><th>FF ID</th><th>Main Click ID</th><th>Status</th><th>Rank Assign</th><th>Actions</th></tr></thead>
        ...
        <div id="admin-view-advertiser-approvals" class="box-container hidden">
            ...
            <div class="table-wrap">
                <table class="data-table">
                    <thead><tr><th>Advertiser ID</th><th>Method</th><th>TxID</th><th>Receipt</th><th>Actions</th></tr></thead>
                    <tbody id="admin-advertiser-approvals-body">
                        <tr>
                            <td>ADV-1092</td>
                            <td>Bitcoin (BTC)</td>
                            <td><code>tx_btc_984572938475</code></td>
                            <td><a href="#" onclick="showToast('Sample receipt view'); return false;" style="color:#3699ff;">View Screenshot</a></td>
                            <td style="display:flex; gap:5px;">
                                <button class="btn-submit" style="margin-top:0; padding: 4px 8px; font-size: 11px; background: #1bc5bd;" onclick="showToast('Deposit approved & balance updated!','success')">Approve</button>
                                <button class="btn-submit" style="margin-top:0; padding: 4px 8px; font-size: 11px; background: #f64e60;" onclick="showToast('Deposit rejected','info')">Reject</button>
                            </td>
                        </tr>
                    </tbody>
                </table>
        ...
        <div id="admin-view-live-earnings" class="box-container hidden">
            ...
            <div class="table-wrap">
                <table class="data-table">
                    <thead><tr><th>Affiliate</th><th>Clicks</th><th>Leads</th><th>Earnings</th><th>EPC</th></tr></thead>
        ...
        <div id="admin-view-click-tracking" class="box-container hidden">
            <div class="box-header">
                ...
                <input type="text" id="admin-ct-search" class="form-control" placeholder="Search by affiliate email..." style="width: 260px; background: var(--bg-dark);" maxlength="120">
            </div>
            <div class="metrics-grid-2x2" style="margin-bottom: 20px;">
            ...
            <div class="table-wrap">
                <table class="data-table">
                    <thead><tr><th>Affiliate</th><th>Total Clicks</th><th>Last Click Time</th><th>Click ID(s)</th><th>Per-Click-ID Count</th><th>Last Click At (Per ID)</th></tr></thead>
        ...
        <div id="admin-view-traffic-quality" class="box-container hidden">
            ...
            <div class="table-wrap">
                <table class="data-table">
                    <thead><tr><th>Affiliate</th><th>Total Clicks</th><th>Unique IPs</th><th>Duplicate IPs</th><th>Duplicate Rate</th><th>Leads</th><th>Lead Quality</th></tr></thead>
        ...
        <div id="admin-view-managers" class="box-container hidden">
            ...
            <div style="background: var(--bg-dark); padding: 20px; border-radius: 6px; border: 1px solid var(--border-color); margin-bottom: 20px;">
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="email" id="manager-add-email" class="form-control" placeholder="manager@admadia.site" style="background: var(--card-bg);" maxlength="254">
                    </div>
                    <div class="form-group">
                        <label>Manager Password (min 8 chars)</label>
                        <input type="text" id="manager-add-password" class="form-control" placeholder="Set a password" style="background: var(--card-bg);" maxlength="128" autocomplete="new-password">
                    </div>
                </div>
            ...
            <div class="table-wrap">
                <table class="data-table">
                    <thead><tr><th>Manager Email</th><th>Created</th><th>Actions</th></tr></thead>
    ...
    <div class="sticky-top-header">
        <div class="header-metrics-group">
            ...
            <div class="stat-circular-card">
                <div class="progress-circle red-circle" style="--pgPercentage: 0;" id="circle-month"><div class="circle-inner" id="percent-month">0%</div></div>
                <div class="stat-info">
            ...
            <div class="stat-circular-card">
                <div class="progress-circle blue-circle" style="--pgPercentage: 0;" id="circle-week"><div class="circle-inner" id="percent-week">0%</div></div>
                <div class="stat-info">
            ...
            <div class="stat-circular-card">
                <div class="progress-circle red-circle" style="--pgPercentage: 0;" id="circle-day"><div class="circle-inner" id="percent-day">0%</div></div>
                <div class="stat-info">
            ...
            <div class="stat-circular-card">
                <div class="progress-circle blue-circle" style="--pgPercentage: 100;" id="circle-hour"><div class="circle-inner" id="percent-hour">100%</div></div>
                <div class="stat-info">
    ...
    <div class="main-wrapper">
                ...
                <div class="left-column">
                    ...
                    <div class="box-container" id="affiliate-chat-box">
                        ...
                        <div class="chat-input-row" id="affiliate-chat-input-row">
                            <input type="text" id="affiliate-chat-input" placeholder="Type a message to the admin..." maxlength="2000">
                ...
                <div class="right-column">
                    ...
                    <div class="box-container" style="margin-bottom: 20px;">
                        ...
                        <div style="margin-bottom: 12px; background: var(--bg-dark); padding: 10px 12px; border-radius: 6px; border: 1px solid var(--border-color); display: flex; justify-content: space-between; align-items: center;">
                            <a id="preview-global-link" href="https://app.trcefy.com/click?pid=2&offer_id=23972&sub2=u814724&sub5=s1" target="_blank" rel="noopener noreferrer" style="color: #3699ff; text-decoration: none; font-weight: 500; font-size: 13px;">Master Global Smart Link Rotator</a>
            ...
            <div class="box-container">
                ...
                <div style="background: var(--bg-dark); padding: 20px; border-radius: 6px; border: 1px solid var(--border-color); margin-bottom: 25px;">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="campaign-name-input" class="form-control" placeholder="My First Campaign" style="background: var(--card-bg);" maxlength="120">
                    </div>
        ...
        <div id="view-balance" class="hidden">
            <div class="box-container">
                ...
                <div class="table-wrap">
                    <table class="data-table">
                        <thead><tr><th>Invoice ID</th><th>Created Date (Monday)</th><th>Amount</th><th>Method</th><th>Status</th></tr></thead>
            ...
            <div class="box-container">
                ...
                <div style="display: flex; gap: 15px; margin-bottom: 20px; flex-wrap: wrap; align-items: end;">
                    ...
                    <div class="form-group" style="margin-bottom:0;">
                        ...
                        <input type="text" id="report-click-id" class="form-control" placeholder="Search by Click ID" style="background: var(--card-bg); width: 200px;" maxlength="120">
                    </div>
                    <button class="btn-submit" id="btn-apply-report-filter" style="margin-top:0; width: 120px;">Apply Filter</button>
                </div>
                <div class="table-wrap">
                    <table class="data-table">
                        <thead><tr><th>Timestamp</th><th>Click ID</th><th>Campaign</th><th>IP</th><th>Conversion</th><th>Earnings</th></tr></thead>
        ...
        <div id="view-referral" class="hidden">
            <div class="box-container">
                ...
                <div style="background: var(--bg-dark); padding: 25px; border-radius: 6px; border: 1px solid var(--border-color);">
                    ...
                    <p style="color: var(--text-muted); font-size: 13px; margin-bottom: 10px;">Enter a promo code shared with you. Redemptions are reviewed and credited by an admin.</p>
                    <div style="display: flex; gap: 10px; margin-bottom: 25px;">
                        <input type="text" id="promo-code-input" class="form-control" placeholder="Enter promo code..." style="background: var(--card-bg); color: var(--text-main);" maxlength="40">
                    ...
                    <div style="background: var(--card-bg); padding: 15px; border-radius: 6px; border: 1px solid var(--border-color);">
                        <table class="data-table">
                           <thead><tr><th>Referred User Email</th><th>Join Date</th><th>Commission Generated</th></tr></thead>
            ...
            <div class="box-container">
                ...
                <div style="background: var(--bg-dark); padding: 20px; border-radius: 6px; border: 1px solid var(--border-color);">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" style="background: var(--card-bg);" value="https://app.trcefy.com/click?pid=2&offer_id=23972&sub2=u814724&sub5=s1clickid={clickid}&payout={payout}" readonly>
                    </div>
            ...
            <div class="box-container">
                ...
                <div style="background: var(--bg-dark); padding: 25px; border-radius: 6px; border: 1px solid var(--border-color); display: flex; gap: 30px; align-items: center; flex-wrap: wrap;">
                    <div style="text-align: center;">
                        ...
                        <br>
                        <input type="text" id="input-avatar-url" class="form-control" placeholder="Avatar Image URL" style="width: 220px; font-size: 11px; margin-top: 5px;" maxlength="500">
                    ...
                    <div style="flex-grow: 1;">
                        <div class="form-group">
                            ...
                            <input type="text" id="profile-input-name" class="form-control" style="background: var(--card-bg);" value="Affiliate User" maxlength="120">
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="profile-input-btc" class="form-control" style="background: var(--card-bg);" placeholder="bc1qxy2kgdygjrsqtzq2n0yrf2493p83kkfjhx0wlh" maxlength="120">
                        </div>
            ...
            <div class="box-container">
                ...
                <div style="background: var(--bg-dark); padding: 25px; border-radius: 6px; border: 1px solid var(--border-color);">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="adv-btc-txid" class="form-control" placeholder="Enter Bitcoin TxID..." style="background: var(--card-bg);" maxlength="120">
                    </div>
                    <div class="form-group">
                        <label>Payment Receipt Screenshot</label>
                        <input type="file" id="adv-btc-receipt" class="form-control" style="background: var(--card-bg); padding: 8px;" accept="image/*">
                    </div>
        ...
        <div id="view-adv-campaigns" class="hidden">
            <div class="box-container">
                ...
                <div style="background: var(--bg-dark); padding: 20px; border-radius: 6px; border: 1px solid var(--border-color);">
                    ...
                    <button class="btn-submit" style="width: 200px; margin-bottom: 20px;" onclick="showToast('New campaign creator is not implemented yet','info')">+ Create New Campaign</button>
                    <table class="data-table">
                        <thead><tr><th>Campaign ID</th><th>Title</th><th>Budget</th><th>Status</th></tr></thead>
                        <tbody>
                            <tr><td>CMP-5011</td><td>Global App Install Push</td><td>$500.00</td>
                            <td><span style="color: #1bc5bd; font-weight: bold;">Running 🟢</span></td></tr>
                        </tbody>
                    </table>
                </div>
            </div>
...
<div id="offer-modal" class="modal-overlay hidden">
    <div class="modal-box">
        ...
        <div>
            ...
            <div class="modal-detail-row"><span class="label">Preview URL:</span><a id="modal-preview-url" href="#" target="_blank" rel="noopener noreferrer" style="color: #3699ff; text-decoration: none;">Click to Preview</a></div>
...
<div id="admin-offer-modal" class="modal-overlay hidden">
    <div class="modal-box">
        ...
        <div class="form-group">
            <label>Offer Name</label>
            <input type="text" id="edit-offer-name" class="form-control" style="background: var(--bg-dark);" maxlength="160">
        </div>
        <div class="form-row">
            <div class="form-group">
                <label>Country</label>
                <input type="text" id="edit-offer-country" class="form-control" style="background: var(--bg-dark);" maxlength="8">
            </div>
            <div class="form-group">
                <label>Category</label>
        ...
        <div class="form-row">
            <div class="form-group">
                <label>Payout</label>
                <input type="text" id="edit-offer-payout" class="form-control" style="background: var(--bg-dark);" maxlength="40">
            </div>
            <div class="form-group">
                <label>EPC</label>
                <input type="text" id="edit-offer-epc" class="form-control" style="background: var(--bg-dark);" maxlength="40">
            </div>
        </div>
        <div class="form-group">
            <label>Base Tracking URL</label>
            <input type="text" id="edit-offer-url" class="form-control" style="background: var(--bg-dark);" maxlength="2000">
        </div>
...
<script type="module">
    /* ============================================================
     *  AdMadia — client application
     *  ------------------------------------------------------------
     *  ⚠️  READ ME FIRST — HOW TO CREATE THE FIRST ADMIN
     *  ------------------------------------------------------------
     *  This file NO LONGER ships hard-coded admin credentials.
     *  (The previous version XOR-"obfuscated" the admin email and
     *  password in the client bundle — that is reversible in seconds
     *  by anyone who opens DevTools, so it has been removed.)
     *
     *  To create the first administrator:
     *
     *   1. Firebase Console → Authentication → Users → "Add user"
     *        email:    admin@yourdomain.com
     *        password: <a long, random, unique password>
     *   2. Copy the generated UID.
     *   3. Firebase Console → Realtime Database → Data, add:
     *        admins/<that-UID> : true
     *   4. Deploy the security rules below (Realtime Database → Rules).
     *   5. Log in through the normal login form with that email/password.
     *
     *  ------------------------------------------------------------
     *  WHY THE API KEY IS VISIBLE
     *  ------------------------------------------------------------
     *  A Firebase Web API key is a *public project identifier*, not a
     *  secret. It is safe to ship in client code. All real protection
     *  comes from (a) Firebase Authentication and (b) the Realtime
     *  Database Security Rules below. If those rules are permissive,
     *  the whole database is world-readable/writable no matter how the
     *  client code is written — so the rules MUST be applied.
     *
     *  ------------------------------------------------------------
     ...
     *       "$emailKey": {
     *         ".read":  "auth != null && auth.token.email.replace(/[.#$\\/\\[\\]]/g,'_') === $emailKey",
     *         ".write": "auth != null && root.child('admins').child(auth.uid).exists()",
     ...
     *
     *     "users": {
     *       ".read": "auth != null && (root.child('admins').child(auth.uid).exists() || root.child('managers').child(auth.token.email.replace(/[.#$\\/\\[\\]]/g,'_')).exists())",
     *       "$key": {
     *         ".read": "auth != null && auth.token.email.replace(/[.#$\\/\\[\\]]/g,'_') === $key",
     *
     *         "profile": {
     *           ".read":  "auth != null && (root.child('admins').child(auth.uid).exists() || root.child('managers').child(auth.token.email.replace(/[.#$\\/\\[\\]]/g,'_')).exists() || auth.token.email.replace(/[.#$\\/\\[\\]]/g,'_') === $key)",
     *           ".write": "auth != null && (root.child('admins').child(auth.uid).exists() || root.child('managers').child(auth.token.email.replace(/[.#$\\/\\[\\]]/g,'_')).exists() || auth.token.email.replace(/[.#$\\/\\[\\]]/g,'_') === $key)",
     *           "status": {
     *             ".validate": "root.child('admins').child(auth.uid).exists() || root.child('managers').child(auth.token.email.replace(/[.#$\\/\\[\\]]/g,'_')).exists() || newData.val() === 'Pending Approval' || (data.exists() && newData.val() === data.val())"
     *           },
     *           "rank": {
     *             ".validate": "root.child('admins').child(auth.uid).exists() || root.child('managers').child(auth.token.email.replace(/[.#$\\/\\[\\]]/g,'_')).exists() || !data.exists() || newData.val() === data.val()"
     *           },
     *           "ffid":       { ".validate": "newData.isString() && newData.val().length < 64" },
     *           "email":      { ".validate": "newData.isString() && newData.val().length < 254" },
     *           "name":       { ".validate": "newData.isString() && newData.val().length < 160" },
     *           "btcWallet":  { ".validate": "newData.isString() && newData.val().length < 120" },
     *           "avatar":     { ".validate": "newData.isString() && newData.val().length < 600" },
     *           "mainClickId":{ ".validate": "newData.isString() && newData.val().length < 80" }
     *         },
     *
     *         "stats": {
     *           ".read":  "auth != null && (root.child('admins').child(auth.uid).exists() || root.child('managers').child(auth.token.email.replace(/[.#$\\/\\[\\]]/g,'_')).exists() || auth.token.email.replace(/[.#$\\/\\[\\]]/g,'_') === $key)",
     *           ".write": "auth != null && (root.child('admins').child(auth.uid).exists() || root.child('managers').child(auth.token.email.replace(/[.#$\\/\\[\\]]/g,'_')).exists() || auth.token.email.replace(/[.#$\\/\\[\\]]/g,'_') === $key)",
     *           "clicks":          { ".validate": "newData.isNumber() && newData.val() >= 0" },
     *           "uniqueClicks":    { ".validate": "newData.isNumber() && newData.val() >= 0" },
     *           "duplicateClicks": { ".validate": "newData.isNumber() && newData.val() >= 0" },
     *           "leads":   { ".validate": "newData.isNumber() && newData.val() >= 0 && (root.child('admins').child(auth.uid).exists() || root.child('managers').child(auth.token.email.replace(/[.#$\\/\\[\\]]/g,'_')).exists() || !data.exists() || newData.val() === data.val())" },
     *           "earnings":{ ".validate": "newData.isNumber() && newData.val() >= 0 && (root.child('admins').child(auth.uid).exists() || root.child('managers').child(auth.token.email.replace(/[.#$\\/\\[\\]]/g,'_')).exists() || !data.exists() || newData.val() === data.val())" },
     *           "bonus":   { ".validate": "newData.isNumber() && (root.child('admins').child(auth.uid).exists() || root.child('managers').child(auth.token.email.replace(/[.#$\\/\\[\\]]/g,'_')).exists() || !data.exists() || newData.val() === data.val())" },
     *           "payment": { ".validate": "newData.isNumber() && (root.child('admins').child(auth.uid).exists() || root.child('managers').child(auth.token.email.replace(/[.#$\\/\\[\\]]/g,'_')).exists() || !data.exists() || newData.val() === data.val())" }
     *         },
     *
     *         "referrals": {
     *           ".read":  "auth != null && (root.child('admins').child(auth.uid).exists() || auth.token.email.replace(/[.#$\\/\\[\\]]/g,'_') === $key)",
     *           ".write": "auth != null && (root.child('admins').child(auth.uid).exists() || root.child('managers').child(auth.token.email.replace(/[.#$\\/\\[\\]]/g,'_')).exists())"
     *         }
     *       }
     *     },
     *
     ...
     *       ".read": "auth != null && (root.child('admins').child(auth.uid).exists() || root.child('managers').child(auth.token.email.replace(/[.#$\\/\\[\\]]/g,'_')).exists())",
     *       "$affKey": {
     *         ".read":  "auth != null && auth.token.email.replace(/[.#$\\/\\[\\]]/g,'_') === $affKey",
     *         ".write": "auth != null && (auth.token.email.replace(/[.#$\\/\\[\\]]/g,'_') === $affKey || root.child('admins').child(auth.uid).exists())",
     *         "$eventId": {
     *           ".validate": "newData.hasChildren(['timestamp','clickId'])",
     *           "earnings":  { ".validate": "newData.isNumber() && newData.val() >= 0" },
     *           "conversion":{ ".validate": "newData.isBoolean()" }
     *         }
     *       }
     *     },
     *
     ...
     *       "$affKey": {
     *         ".read":  "auth != null && auth.token.email.replace(/[.#$\\/\\[\\]]/g,'_') === $affKey",
     *         ".write": "auth != null && auth.token.email.replace(/[.#$\\/\\[\\]]/g,'_') === $affKey",
     *         "summary":  { "totalClicks": { ".validate": "newData.isNumber() && newData.val() >= 0" } },
     *         "clickIds": { "$cid": { "count": { ".validate": "newData.isNumber() && newData.val() >= 0" } } }
     *       }
     *     },
     *
     *     "dailyCounters": {
     ...
     *         "$day": {
     *           "clicks":   { ".validate": "newData.isNumber() && newData.val() >= 0" },
     *           "leads":    { ".validate": "newData.isNumber() && newData.val() >= 0" },
     *           "earnings": { ".validate": "newData.isNumber() && newData.val() >= 0" }
     *         }
     *       }
     *     },
     ...
     *         ".read":  "auth != null && auth.token.email.replace(/[.#$\\/\\[\\]]/g,'_') === $affKey",
     *         ".write": "auth != null && auth.token.email.replace(/[.#$\\/\\[\\]]/g,'_') === $affKey",
     *         "$cid": { ".validate": "newData.hasChildren(['name','trackingLink'])" }
     *       }
     *     },
     *
     *     "chat": {
     ...
     *           "from": { ".validate": "newData.val() === 'affiliate' || newData.val() === 'admin'" }
     *         }
     *       }
     *     },
     *
     ...
     *         ".read":  "auth != null && auth.token.email.replace(/[.#$\\/\\[\\]]/g,'_') === $affKey",
     *         ".write": "auth != null && (auth.token.email.replace(/[.#$\\/\\[\\]]/g,'_') === $affKey || root.child('admins').child(auth.uid).exists())",
     *         "lastMessage": { ".validate": "newData.isString() && newData.val().length <= 2000" }
     *       }
     *     },
     *
     *     "offers": {
     ...
     *       ".write": "auth != null && (root.child('admins').child(auth.uid).exists() || root.child('managers').child(auth.token.email.replace(/[.#$\\/\\[\\]]/g,'_')).exists())",
     *       "redirect": {
     *         "targetUrl": { ".validate": "newData.isString() && (newData.val() === '' || newData.val().matches(/^https?:\\/\\//))" }
     *       }
     *     },
     *
     *     "postbacks": {
     ...
     *       ".read":  "auth != null && (root.child('admins').child(auth.uid).exists() || root.child('managers').child(auth.token.email.replace(/[.#$\\/\\[\\]]/g,'_')).exists())",
     *       ".write": "auth != null && (root.child('admins').child(auth.uid).exists() || root.child('managers').child(auth.token.email.replace(/[.#$\\/\\[\\]]/g,'_')).exists())",
     *       "$invId": { "amount": { ".validate": "newData.isNumber() && newData.val() >= 0" } }
     *     },
     *
     *     "leads": {
     *       ".read":  "auth != null",
     *       ".write": "auth != null && (root.child('admins').child(auth.uid).exists() || root.child('managers').child(auth.token.email.replace(/[.#$\\/\\[\\]]/g,'_')).exists())",
     *       "$leadId": { "earnings": { ".validate": "newData.isNumber() && newData.val() >= 0" } }
     *     },
     *
     *     "notifications": {
     *       ".read": "auth != null && (root.child('admins').child(auth.uid).exists() || root.child('managers').child(auth.token.email.replace(/[.#$\\/\\[\\]]/g,'_')).exists())",
     *       "$key": {
     *         ".read":  "auth != null && auth.token.email.replace(/[.#$\\/\\[\\]]/g,'_') === $key",
     *         ".write": "auth != null && (auth.token.email.replace(/[.#$\\/\\[\\]]/g,'_') === $key || root.child('admins').child(auth.uid).exists() || root.child('managers').child(auth.token.email.replace(/[.#$\\/\\[\\]]/g,'_')).exists())",
     *         "$notifId": { "message": { ".validate": "newData.isString() && newData.val().length <= 500" } }
     *       }
     *     },
     *
     *     "promoCodes": {
     *       ".read":  "auth != null",
     *       ".write": "auth != null && (root.child('admins').child(auth.uid).exists() || root.child('managers').child(auth.token.email.replace(/[.#$\\/\\[\\]]/g,'_')).exists())",
     *       "$code": { "reward": { ".validate": "newData.isNumber() && newData.val() >= 0 && newData.val() <= 10000" } }
     *     },
     *
     *     "promoRedemptions": {
     *       ".read": "auth != null && (root.child('admins').child(auth.uid).exists() || root.child('managers').child(auth.token.email.replace(/[.#$\\/\\[\\]]/g,'_')).exists())",
     ...
     *         ".read": "auth != null && auth.token.email.replace(/[.#$\\/\\[\\]]/g,'_') === $key",
     *         "$rid": {
     *           ".write": "auth != null && (auth.token.email.replace(/[.#$\\/\\[\\]]/g,'_') === $key || root.child('admins').child(auth.uid).exists())",
     *           ".validate": "newData.hasChildren(['code','status'])",
     *           "status": { ".validate": "newData.val() === 'pending' || root.child('admins').child(auth.uid).exists() || root.child('managers').child(auth.token.email.replace(/[.#$\\/\\[\\]]/g,'_')).exists()" }
     *         }
     *       }
     *     }
     *
    ...
    /* ---------- Session cache ----------
       Purely a UI convenience. It is NEVER trusted for authorisation:
       every privileged read/write is re-checked by Firebase Security
       Rules against the real Firebase Auth token. */
    const SESSION_KEY = "admadia_session_v1";
    const SESSION_DAYS = 3;

    /* ---------- Hard limits / constants ---------- */
    const MAX_CHAT_LEN     = 2000;
    const MAX_TRACK_PER_MIN = 40;   // client-side click throttle
    const ALLOWED_IMG_HOSTS = ['i.pravatar.cc', 'lh3.googleusercontent.com'];
    ...
    window.showToast = function(message, type='info', duration=3500) {
        ...
        t.textContent = String(message);     // textContent → never parsed as HTML
    ...
    function generateUniqueClickId() {
        // crypto.getRandomValues where available, Math.random fallback
        ...
        if (window.crypto && window.crypto.getRandomValues) {
            ...
            for (let i = 0; i < length; i++) r += chars.charAt(buf[i] % chars.length);
        } else {
    ...
    /** Escape for HTML text *and* HTML attribute contexts. */
    function escapeHtml(s) {
        return String(s == null ? '' : s).replace(/[&<>"']/g, c => ({
            '&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'
        }[c]));
    }
    /** Escape for a single-quoted JS string that lives inside a double-quoted HTML attribute. */
    function escAttr(s) {
    ...
    /** Only allow http/https/relative URLs — blocks javascript:, data:, vbscript: etc. */
    function safeUrl(u, { allowRelative = false } = {}) {
        const s = String(u == null ? '' : u).trim();
        if (!s) return '';
        if (allowRelative && /^\//.test(s)) return s;
        ...
        try {
            ...
            if (!snap.exists()) {
                ...
                await set(pRef, {
                    ...
                    status: 'Pending Approval', rank: 'Standard Affiliate',
                    name: email.split('@')[0]
                });
                return newId;
            }
    ...
    // ---------- Client-side click throttle (defence in depth; real limits belong in rules/Cloud Functions) ----------
    const _trackTimestamps = [];
    function throttleOk() {
        const now = Date.now();
        while (_trackTimestamps.length && now - _trackTimestamps[0] > 60000) _trackTimestamps.shift();
        if (_trackTimestamps.length >= MAX_TRACK_PER_MIN) return false;
        _trackTimestamps.push(now);
        return true;
    }
    ...
    window.trackAndOpenLink = async function(linkUrl, offerId = null) {
        ...
        if (!throttleOk()) {
            showToast('Too many link generations — please slow down.', 'error');
            return;
        }
        ...
        try {
            ...
            clientIp = String(d.ip || 'unknown').slice(0, 60);
        } catch(e) {}
    ...
    function initializeRealtimeSync(key) {
        ...
        onValue(ref(db, `users/${key}`), (snap) => {
            ...
            if (refArray.length > 0) {
                let h = '';
                refArray.forEach(r => {
                    h += `<tr><td>${escapeHtml(r.email||'')}</td><td>${escapeHtml(r.date||'')}</td><td style="font-weight:bold; color:#1bc5bd;">+$15.00</td></tr>`;
                });
    ...
    window.sendAffiliateChat = async function() {
        ...
        if (!text) return;
        if (text.length > MAX_CHAT_LEN) { showToast('Message is too long.', 'error'); return; }
        const key = safeKey(loggedInUserEmail);
    ...
    function renderInboxList(snap) {
        ...
        items.forEach(it => {
            ...
            const unread = it.unreadForAdmin ? `<span style="background:#f64e60; color:#fff; border-radius:10px; padding:1px 7px; font-size:10px; margin-left:6px; font-weight:bold;">${escapeHtml(it.unreadForAdmin)}</span>` : '';
            html += `
    ...
    window.selectAdminInbox = async function(key) {
        ...
        const email = String(key).replace(/_/g, '.');
        header.innerHTML = `
            <div>
    ...
    window.sendAdminInboxMsg = async function() {
        ...
        try {
            ...
            const meta = metaSnap.exists() ? metaSnap.val() : { unreadForAdmin: 0, unreadForAffiliate: 0 };
            await update(metaRef, {
                email: meta.email || String(key).replace(/_/g, '.'),
                lastMessage: text,
                lastTs: Date.now(),
                lastFrom: 'admin',
            ...
            list.innerHTML = items.map(n => `
                <div class="notif-item">
                    ...
                    <div style="font-size:11px; margin-top:4px;">
                        ${escapeHtml(String(n.aff || '').replace(/_/g, '.'))}
    ...
    window.testRedirectPreview = function() {
        const el = document.getElementById('redirect-preview-url');
        if (!el) return;
        const url = safeUrl(el.textContent.trim());
        if (!url) { showToast('Preview URL is not a valid http(s) URL.', 'error'); return; }
    ...
    function renderPostbackTable(snap) {
        ...
        snap.forEach(c => {
            const p = c.val() || {};
            h += `<tr>
                <td>${escapeHtml(p.email || String(c.key).replace(/_/g,'.'))}</td>
    ...
    function initPostbackManager() {
        ...
        document.getElementById('btn-save-global-postback').addEventListener('click', async () => {
            ...
            if (!/^https?:\/\//i.test(url)) { showToast('Postback URL must start with http:// or https://', 'error'); return; }
            try {
    ...
    window.editCustomPostback = function(key, url, email) {
        document.getElementById('custom-postback-email').value = email || String(key).replace(/_/g, '.');
    ...
    function renderInvoiceTable() {
        ...
        const filtered = invoiceRowsCache.filter(inv => {
            ...
            if (q) {
                const hay = (String(inv.id) + ' ' + String(inv.email) + ' ' + String(inv.affKey)).toLowerCase();
        ...
        filtered.forEach(inv => {
            ...
            h += `<tr>
                ...
                <td>${escapeHtml(inv.email || String(inv.affKey).replace(/_/g,'.'))}</td>
    ...
    // ============================================================
    //  LANDING-PAGE NAVIGATION + AUTH VIEW SWITCHING
    // ============================================================
    ...
    function showAuthView(formToShow) {
    ...
    // Wire up landing-page nav links
    ...
    const footContact   = document.getElementById('footContact');
    ...
    function scrollToContact() {
        ...
        if (lv && footer) {
            lv.scrollTo({ top: footer.offsetTop - 40, behavior: 'smooth' });
        }
    }

    if (navHome) navHome.addEventListener('click', (e) => { e.preventDefault(); scrollToTop(); });
    if (navContact) navContact.addEventListener('click', (e) => { e.preventDefault(); scrollToContact(); });
    if (brandHome) brandHome.addEventListener('click', (e) => { e.preventDefault(); scrollToTop(); });
    if (footHome) footHome.addEventListener('click', (e) => { e.preventDefault(); scrollToTop(); });
    if (footContact) footContact.addEventListener('click', (e) => { e.preventDefault(); scrollToContact(); });

    [navLogin, footLogin].forEach(el => el && el.addEventListener('click', () => showAuthView('login')));
    [navRegister, footRegister].forEach(el => el && el.addEventListener('click', () => showAuthView('signup')));
    [backToLanding, backToLanding2].forEach(el => el && el.addEventListener('click', showLandingPage));

    // Landing page: hamburger, sticky shadow, scroll reveal
    (function initLandingInteractions(){
        ...
        if (header) {
            ...
            lv.addEventListener('scroll', () => {
                header.classList.toggle('scrolled', lv.scrollTop > 8);
            }, { passive: true });
        }
        ...
        const revealEls = document.querySelectorAll('#landing-view .reveal');
    ...
    // ============================================================
    //  AUTH UI TOGGLES (inside auth-wrapper)
    // ============================================================
    ...
    /* Advertiser link — previously this granted an unauthenticated
       "advertiser" dashboard with a fake session. That was a privilege
       bypass; it now simply routes to the standard signup form. */
    document.getElementById('advertiserToggleBtn').addEventListener('click', (e) => {
        e.preventDefault();
        showAuthView('signup');
        showToast('Advertiser accounts are created through the standard sign-up form.', 'info', 5000);
    });
    ...
    document.getElementById('btn-signup').addEventListener('click', async () => {
        ...
        if (pw.length < 8) { errEl.innerText = 'Password must be at least 8 characters.'; return; }
    ...
    // ============================================================
    //  SESSION RESTORE / ROLE RESOLUTION
    //  Every role decision is made against the live Firebase auth
    //  token + the database, NOT against localStorage.
    // ============================================================
    onAuthStateChanged(auth, async (user) => {
        ...
        try {
            ...
            } catch (_) { /* not permitted → not an admin */ }
            ...
            try {
                ...
                isManager = mgrSnap.exists() && mgrSnap.val().active !== false;
            } catch (_) { /* not permitted → not a manager */ }
            ...
            const key = safeKey(email);
            let pSnap = null;
            try { pSnap = await get(ref(db, `users/${key}/profile`)); } catch (_) {}

            if (pSnap && pSnap.exists() && pSnap.val().status === 'Approved') {
        ...
        if (loggedInUserEmail) {
            ...
            get(ref(db, `users/${key}/profile`)).then(snap => {
                if (snap.exists()) {
                    ...
                    if (av) {
                        document.getElementById('header-avatar-img').src = av;
                        document.getElementById('profile-view-avatar').src = av;
                    }
                }
            }).catch(()=>{});
        }
    ...
    function switchView(viewId, opts = {}) {
        ...
        if (viewId === 'admin-view-settings')         loadPromoRedemptions();
    ...
    function startLiveRealtimeEngine() {
        ...
        onChildAdded(leadsQ, (child) => {
            ...
            li.innerHTML = `
                <img src="https://i.pravatar.cc/100?img=${Math.floor(Math.random()*70)+1}" class="user-avatar" alt="">
                <div style="flex-grow:1;">
                    <strong style="color: var(--text-main);">Lead (Click ID: ${escapeHtml(lead.clickId || '—')})</strong>
                    <br><span style="font-size: 11px; color: var(--text-muted);">Affiliate: ${escapeHtml(String(lead.affiliate||'').replace(/_/g,'.'))}</span>
                </div>
        ...
        function pushDemo() {
            ...
            const li = document.createElement('li');
            li.innerHTML = `
                <img src="https://i.pravatar.cc/100?img=${av}" class="user-avatar" alt="">
                <div style="flex-grow:1;">
            ...
            html += `
                <li>
                    <div style="display:flex; align-items:center; gap:10px;">
                        <img src="https://i.pravatar.cc/100?img=${(i*3)%70+1}" class="user-avatar" alt="">
                        <div>
    ...
    function renderOfferLists(filterCategory) {
        ...
        for (let j = 0; j < Math.min(5, cachedOffers.length); j++) {
            ...
            dh += `
                <div class="offer-item">
                    <div>
                        <a href="javascript:void(0)" onclick="openOfferModal('${escAttr(o.id)}', '${escAttr(label)}', '${escAttr(o.catLabel)}', '${escAttr(o.url)}')">${escapeHtml(label)}</a>
                        <br><span class="payout-badge">${escapeHtml(o.payout)}</span>
                    </div>
                    <div class="offer-actions">
        ...
        document.getElementById('offerContainer').innerHTML = dh;
            ...
            ah += `
                <div class="new-offer-card">
                    <div class="offer-card-header">
                        <div><button class="offer-title-btn" onclick="openOfferModal('${escAttr(o.id)}', '${escAttr(label)}', '${escAttr(o.catLabel)}', '${escAttr(o.url)}')">${escapeHtml(label)}</button></div>
                        <div class="offer-badges-top">
                            <span class="badge-ui b-id">ID: ${escapeHtml(o.id)}</span>
                            <span class="badge-ui b-country">Geo: ${escapeHtml(o.country)}</span>
                            <span class="badge-ui b-epc">EPC: ${escapeHtml(o.epc)}</span>
                        </div>
                    </div>
                    <div class="offer-card-footer">
                        <div style="display:flex; gap:10px; align-items:center;">
                            <span class="payout-badge">${escapeHtml(o.payout)}</span>
                            <span class="${o.statusClass}">${escapeHtml(o.statusText)}</span>
                        </div>
                        <button class="btn-getlink" onclick="trackAndOpenLink('${escAttr(o.url)}', '${escAttr(o.id)}')">Get Link / Copy</button>
                    </div>
                </div>`;
        });
        document.getElementById('allOffersContainer').innerHTML = ah;
    }

    window.openOfferModal = function(id, name, cat, url) {
        document.getElementById('modal-title').innerText = String(name);
        document.getElementById('modal-id').innerText = String(id);
        document.getElementById('modal-category').innerText = String(cat);
        const safe = safeUrl(url);
        document.getElementById('modal-preview-url').href = safe || '#';
        document.getElementById('modal-track-link').value = buildFinalLink(safe || masterGlobalLink);
    ...
    document.getElementById('btn-create-campaign').addEventListener('click', async () => {
        ...
        if (name.length > 120) { showToast('Campaign name is too long.', 'error'); return; }
        if (!loggedInUserEmail) return;
        const key = safeKey(loggedInUserEmail);
    ...
    async function loadCampaignsList(key) {
        ...
        onValue(ref(db, `campaigns/${key}`), (snap) => {
                ...
                h += `
                    <div style="border-bottom:1px solid var(--border-color); padding:12px 0; display:flex; justify-content:space-between; align-items:center;">
                        <div>
                            ...
                            <br><span style="font-size:12px; color:var(--text-muted);">Base: ${escapeHtml(camp.trackingLink)}</span>
                            <br><span style="font-size:12px; color:var(--text-muted);">ID: <code>${escapeHtml(camp.clickId)}</code></span>
                        </div>
            ...
            container.innerHTML = `<div style="text-align:center; color:#f64e60;">Error: ${escapeHtml(err.message)}</div>`;
        });
    }
    ...
    document.getElementById('btn-apply-report-filter').addEventListener('click', () => {
        ...
        get(ref(db, `trackings/${key}`)).then(snap => {
            ...
            snap.forEach(c => {
                ...
                if (d.timestamp >= sTs && d.timestamp <= eTs) {
                    ...
                    h += `<tr>
                        ...
                        <td><code>${escapeHtml(d.clickId||c.key)}</code></td>
                        <td>${escapeHtml(d.campaignId||'Global')}</td>
                        <td>${escapeHtml(d.ip||'N/A')}</td>
            ...
            tbody.innerHTML = `<tr><td colspan="6" style="text-align:center; color:#f64e60; padding:15px;">Error: ${escapeHtml(e.message)}</td></tr>`;
        });
    });
    ...
    window.updateProfileAvatar = async function() {
        const raw = document.getElementById('input-avatar-url').value.trim();
        if (!raw || !loggedInUserEmail) return;
        const url = safeUrl(raw);
        if (!url) { showToast('Avatar URL must start with http:// or https://', 'error'); return; }
        ...
        try {
            ...
            showToast('Failed to save avatar: ' + (e.message || 'error'), 'error');
        }
    };
    window.saveProfileChanges = async function() {
        if (!loggedInUserEmail) return;
        const name = document.getElementById('profile-input-name').value.trim().slice(0, 120);
        const btc = document.getElementById('profile-input-btc').value.trim().slice(0, 120);
        if (btc && !/^[a-zA-Z0-9]{20,120}$/.test(btc)) {
            showToast('Bitcoin wallet address looks invalid.', 'error');
            return;
        }
        try {
    ...
    /* Promo redemption is now a REQUEST, not a self-credit.
       Previously an affiliate could write directly to their own
       stats.earnings — i.e. mint themselves money. The client now only
       records a pending request; an admin approves it. */
    window.redeemPromoCode = async function() {
        const code = document.getElementById('promo-code-input').value.trim();
        if (!code || !loggedInUserEmail) { showToast('Enter a promo code.', 'error'); return; }
        if (code.length > 40) { showToast('Invalid promo code.', 'error'); return; }
        const pref = ref(db, `promoCodes/${code}`);
        let snap;
        try { snap = await get(pref); } catch(e) { showToast('Could not verify promo code.', 'error'); return; }
        if (!snap.exists()) { showToast('Invalid promo code.', 'error'); return; }
        const data = snap.val() || {};
        if (data.used) { showToast('Promo code already used.', 'error'); return; }
        const reward = Math.min(Math.max(Number(data.reward || 10), 0), 10000);

        const key = safeKey(loggedInUserEmail);
        const rid = 'PR-' + Date.now() + '-' + Math.random().toString(36).slice(2, 7);
        try {
            await set(ref(db, `promoRedemptions/${key}/${rid}`), {
                code, reward,
                email: loggedInUserEmail,
                status: 'pending',
                requestedAt: Date.now()
            });
            showToast('Promo submitted — an admin will review and credit your account.', 'success', 5000);
            document.getElementById('promo-code-input').value = '';
        } catch (e) {
            showToast('Could not submit promo: ' + (e.message || 'error'), 'error');
        }
    };
    ...
    window.submitBitcoinDeposit = function() {
        ...
        if (!/^[a-zA-Z0-9]{10,120}$/.test(tx)) { showToast('TxID format looks invalid.', 'error'); return; }
    ...
    function loadAdminUsersList() {
        ...
        onValue(ref(db, 'users'), (snap) => {
            ...
            snap.forEach(child => {
                ...
                h += `
                    <tr>
                        <td style="cursor:pointer; color:#3699ff; text-decoration:underline;" onclick="openAdminUserModal('${escAttr(key)}')">${escapeHtml(p.userId || 'AFF-000')}</td>
                        <td>${escapeHtml(p.name || p.firstName || 'User')}</td>
                        <td>${escapeHtml(p.email || key.replace(/_/g,'.'))}</td>
                        <td>${escapeHtml(p.ffid || '—')}</td>
                        <td><code style="color:#1bc5bd;">${escapeHtml(p.mainClickId || '—')}</code></td>
            ...
            tbody.innerHTML = `<tr><td colspan="8" style="text-align:center; color:#f64e60; padding:15px;">Error: ${escapeHtml(err.message)}</td></tr>`;
        });
    }
    ...
    window.approveUser = async function(key) {
        try {
            ...
            showToast('Approve failed: ' + (e.message || 'error'), 'error');
        }
    };
    window.declineUser = async function(key) {
        try {
            ...
            showToast('Decline failed: ' + (e.message || 'error'), 'error');
        }
    };
    window.updateUserRank = async function(key, rank) {
        try {
            ...
            showToast('Rank update failed: ' + (e.message || 'error'), 'error');
        }
    };
    ...
    window.openAdminUserModal = async function(key) {
        ...
        try {
            ...
            if (!snap.exists()) { body.textContent = 'User not found.'; return; }
            const d = snap.val();
            const p = d.profile || {};
            const s = d.stats || {};
            title.innerText = p.name || p.email || key;
            body.innerHTML = `
                <div class="modal-detail-row"><span class="label">User ID</span><span class="val">${escapeHtml(p.userId||'—')}</span></div>
                <div class="modal-detail-row"><span class="label">Full Name</span><span class="val">${escapeHtml((p.firstName||'') + ' ' + (p.lastName||'') || p.name || '—')}</span></div>
                <div class="modal-detail-row"><span class="label">Username</span><span class="val">${escapeHtml(p.username||'—')}</span></div>
                <div class="modal-detail-row"><span class="label">Email</span><span class="val">${escapeHtml(p.email||key)}</span></div>
                <div class="modal-detail-row"><span class="label">FF ID</span><span class="val">${escapeHtml(p.ffid||'—')}</span></div>
                <div class="modal-detail-row"><span class="label">Main Click ID</span><span class="val" style="color:#1bc5bd; font-weight:bold;">${escapeHtml(p.mainClickId||'—')}</span></div>
                <div class="modal-detail-row"><span class="label">Country</span><span class="val">${escapeHtml(p.country||'—')}</span></div>
                <div class="modal-detail-row"><span class="label">Street</span><span class="val">${escapeHtml(p.street||'—')}</span></div>
                <div class="modal-detail-row"><span class="label">State / City / Zip</span><span class="val">${escapeHtml(p.state||'')} / ${escapeHtml(p.city||'')} / ${escapeHtml(p.zip||'')}</span></div>
                <div class="modal-detail-row"><span class="label">Promo Code</span><span class="val">${escapeHtml(p.promoCode||'—')}</span></div>
                <div class="modal-detail-row"><span class="label">Traffic Sources</span><span class="val">${escapeHtml(p.traffic||'—')}</span></div>
                <div class="modal-detail-row"><span class="label">Networks</span><span class="val">${escapeHtml(p.networks||'—')}</span></div>
                <div class="modal-detail-row"><span class="label">BTC Wallet</span><span class="val">${escapeHtml(p.btcWallet||'—')}</span></div>
                <div class="modal-detail-row"><span class="label">Status</span><span class="val">${escapeHtml(p.status||'—')}</span></div>
                <div class="modal-detail-row"><span class="label">Rank</span><span class="val">${escapeHtml(p.rank||'—')}</span></div>
                <div class="modal-detail-row"><span class="label">Clicks</span><span class="val">${Number(s.clicks||0)}</span></div>
                <div class="modal-detail-row"><span class="label">Unique Clicks</span><span class="val">${Number(s.uniqueClicks||0)}</span></div>
                <div class="modal-detail-row"><span class="label">Duplicate Clicks</span><span class="val">${Number(s.duplicateClicks||0)}</span></div>
                <div class="modal-detail-row"><span class="label">Leads</span><span class="val">${Number(s.leads||0)}</span></div>
    ...
    function loadAdminOffersList() {
        ...
        onValue(ref(db, 'offers'), (snap) => {
            ...
            tbody.innerHTML = `<tr><td colspan="6" style="color:#f64e60; padding:15px;">Error: ${escapeHtml(err.message)}</td></tr>`;
        });
    }
    ...
    document.getElementById('btn-offer-add').addEventListener('click', async () => {
        ...
        if (!/^https?:\/\//i.test(url)) { showToast('Tracking URL must start with http:// or https://', 'error'); return; }
        const marker = 'sub5=s1';
        ...
        try {
            ...
            showToast('Add failed: ' + (e.message || 'error'), 'error');
        }
    });
    ...
    document.getElementById('btn-offer-save-edit').addEventListener('click', async () => {
        ...
        if (url && !/^https?:\/\//i.test(url)) { showToast('Tracking URL must start with http:// or https://', 'error'); return; }
        const marker = 'sub5=s1';
        const idx = url.indexOf(marker);
        if (idx !== -1) url = url.substring(0, idx + marker.length);
        try {
    ...
    function renderCtTable() {
        ...
        ctAllRows.forEach(r => {
            ...
            html += `<tr>
                <td>${escapeHtml(r.affKey.replace(/_/g,'.'))}</td>
                <td><strong>${Number(r.affTotal)}</strong></td>
                <td>${r.affLast ? new Date(r.affLast).toLocaleString() : '—'}</td>
                <td><code style="color:#3699ff;">${escapeHtml(r.cid)}</code></td>
                <td><span class="ct-badge ok">${Number(r.cidCount)} clicks</span></td>
    ...
    function loadManagersList() {
        ...
        onValue(ref(db, 'managers'), (snap) => {
            ...
            snap.forEach(c => {
                const m = c.val();
                h += `<tr>
                    <td>${escapeHtml(m.email || String(c.key).replace(/_/g,'.'))}</td>
            ...
            tb.innerHTML = `<tr><td colspan="3" style="color:#f64e60; padding:15px;">Error: ${escapeHtml(err.message)}</td></tr>`;
        });
    }
    document.getElementById('btn-manager-add').addEventListener('click', async () => {
        ...
        const password = document.getElementById('manager-add-password').value;
        ...
        try {
            ...
            showToast('Could not save manager record: ' + (e.message || 'error'), 'error');
        }
    });
    window.deleteManager = async function(key) {
        if (!confirm('Delete this manager? (The Firebase Auth user, if any, must be removed manually from the Firebase Console.)')) return;
        try {
    ...
    // ============================================================
    //  ADMIN — PROMO REDEMPTION APPROVALS
    // ============================================================
    let promoRedemptionsAttached = false;
    function loadPromoRedemptions() {
        const tb = document.getElementById('admin-promo-redemptions-body');
        if (!tb) return;
        if (promoRedemptionsAttached) return;
        promoRedemptionsAttached = true;

        onValue(ref(db, 'promoRedemptions'), (snap) => {
            if (!snap.exists()) {
                tb.innerHTML = '<tr><td colspan="6" style="text-align:center; color: var(--text-muted); padding: 15px;">No redemption requests.</td></tr>';
                return;
            }
            const rows = [];
            snap.forEach(affSnap => {
                affSnap.forEach(rSnap => {
                    const r = rSnap.val() || {};
                    rows.push({ affKey: affSnap.key, rid: rSnap.key, ...r });
                });
            });
            ...
            let h = '';
            rows.forEach(r => {
                const cls = r.status === 'approved' ? 'inv-paid' : (r.status === 'rejected' ? 'inv-void' : 'inv-unpaid');
                h += `<tr>
                    <td>${escapeHtml(r.email || String(r.affKey).replace(/_/g,'.'))}</td>
                    <td><code style="color:#8950fc;">${escapeHtml(r.code||'')}</code></td>
                    <td style="font-weight:700; color:#1bc5bd;">$${Number(r.reward||0).toFixed(2)}</td>
                    <td>${r.requestedAt ? new Date(r.requestedAt).toLocaleString() : '—'}</td>
                    <td><span class="inv-badge ${cls}">${escapeHtml(r.status||'pending')}</span></td>
                    <td style="display:flex; gap:5px; flex-wrap:wrap;">
                        ${r.status === 'pending' ? `
                            <button class="btn-submit" style="margin-top:0; padding:4px 8px; font-size:11px; background:#1bc5bd;"
                                onclick="approvePromoRedemption('${escAttr(r.affKey)}','${escAttr(r.rid)}')">Approve</button>
                            <button class="btn-submit" style="margin-top:0; padding:4px 8px; font-size:11px; background:#f64e60;"
                                onclick="rejectPromoRedemption('${escAttr(r.affKey)}','${escAttr(r.rid)}')">Reject</button>
                        ` : '—'}
                    </td>
                </tr>`;
            });
            ...
            tb.innerHTML = `<tr><td colspan="6" style="color:#f64e60; padding:15px;">Error: ${escapeHtml(err.message)}</td></tr>`;
        });
    }

    window.approvePromoRedemption = async function(affKey, rid) {
        try {
            const rSnap = await get(ref(db, `promoRedemptions/${affKey}/${rid}`));
            if (!rSnap.exists()) { showToast('Request not found.', 'error'); return; }
            const r = rSnap.val() || {};
            if (r.status !== 'pending') { showToast('Request already processed.', 'info'); return; }
            const reward = Math.min(Math.max(Number(r.reward || 0), 0), 10000);

            const statsRef = ref(db, `users/${affKey}/stats`);
            const sSnap = await get(statsRef);
            const s = sSnap.exists() ? sSnap.val() : {};
            await update(statsRef, {
                earnings: Number(s.earnings || 0) + reward,
                bonus: Number(s.bonus || 0) + reward
            });
            await update(ref(db, `promoRedemptions/${affKey}/${rid}`), {
                status: 'approved', approvedAt: Date.now(), approvedBy: loggedInUserEmail
            });
            if (r.code) {
                await update(ref(db, `promoCodes/${r.code}`), {
                    used: true, usedBy: r.email || affKey, usedAt: Date.now()
                }).catch(()=>{});
            }
            showToast('Promo approved and credited.', 'success');
        } catch (e) {
            showToast('Approve failed: ' + (e.message || 'error'), 'error');
        }
    };
    window.rejectPromoRedemption = async function(affKey, rid) {
        try {
            await update(ref(db, `promoRedemptions/${affKey}/${rid}`), {
                status: 'rejected', rejectedAt: Date.now(), rejectedBy: loggedInUserEmail
            });
            showToast('Promo request rejected.', 'info');
        } catch (e) {
    ...
    document.getElementById('btn-admin-search-click-id').addEventListener('click', async () => {
        ...
        try {
            ...
            res.innerHTML = `
                <div style="background: var(--card-bg); padding: 15px; border-radius: 6px; border: 1px solid var(--border-color);">
                    ...
                    Click ID: <code>${escapeHtml(cid)}</code><br>
                    Event ID: <code>${escapeHtml(evId)}</code><br>
                    Affiliate: ${escapeHtml(String(aff).replace(/_/g,'.'))}<br>
                    IP: ${escapeHtml(found.ip||'N/A')}<br>
        ...
        } catch(e) { res.innerHTML = `<span style="color:#f64e60;">Error: ${escapeHtml(e.message)}</span>`; }
    });
    window.approveLeadByClickId = async function(evId, affKey) {
        try {
            ...
            await update(statsRef, { earnings: Number(s.earnings||0)+25, leads: Number(s.leads||0)+1 });
            const day = todayKey();
            ...
            await set(dRef, { clicks: d.clicks||0, leads: Number(d.leads||0)+1, earnings: Number(d.earnings||0)+25 });
            ...
            showToast('Approve lead failed: ' + (e.message || 'error'), 'error');
        }
    };
    ...
    document.getElementById('btn-save-api-settings').addEventListener('click', () => {
        const api = document.getElementById('admin-api-key-input').value.trim();
        let link = document.getElementById('admin-master-link-input').value.trim();
        if (link && !/^https?:\/\//i.test(link)) { showToast('Master link must start with http:// or https://', 'error'); return; }
        currentApiKey = api;
        const marker = 's1';
        const idx = link.indexOf(marker);
        if (idx !== -1) link = link.substring(0, idx + marker.length);
        masterGlobalLink = link;
        const pg = document.getElementById('preview-global-link');
        if (pg) pg.href = safeUrl(masterGlobalLink) || '#';
        generateOffersData();
        updateRedirectPreview();
        showToast('API settings saved.', 'success');
    });

    /* Previously this switched the client into a fake "affiliate" session
       with no authentication and no database permissions — confusing and
       a UI-level privilege bypass. It now only works if the current user
       genuinely has an affiliate profile. */
    document.getElementById('btn-admin-switch-affiliate').addEventListener('click', async () => {
        const target = prompt('Enter the affiliate email you want to preview (must have an approved profile):');
        if (!target) return;
        const key = safeKey(target.trim());
        try {
            const snap = await get(ref(db, `users/${key}/profile`));
            if (!snap.exists()) { showToast('No such affiliate profile.', 'error'); return; }
            const p = snap.val();
            loggedInUserEmail = target.trim();
            loggedInUserFFId = p.ffid || 'FF-00000';
            loggedInUserMainClickId = p.mainClickId || '';
            currentRole = 'affiliate';
            showAffiliateDashboard();
        } catch (e) {
            showToast('Preview failed: ' + (e.message || 'error'), 'error');
        }
    });
    ...
    document.getElementById('btn-admin-manual-update').addEventListener('click', async () => {
        ...
        if (!target || !Number.isFinite(amount)) { showToast('Invalid input.', 'error'); return; }
        if (Math.abs(amount) > 1000000) { showToast('Amount out of range.', 'error'); return; }
        const key = safeKey(target);
        try {
            const statsRef = ref(db, `users/${key}/stats`);
            const snap = await get(statsRef);
            let s = snap.exists() ? snap.val() : { clicks: 0, leads: 0, earnings: 0 };
            if (action === 'add_balance') s.earnings = Number(s.earnings||0) + amount;
            else if (action === 'add_lead') { s.leads = Number(s.leads||0) + amount; s.earnings = Number(s.earnings||0) + (amount * 25); }
            await update(statsRef, s);
            showToast(`Account updated for ${target}.`, 'success');
        } catch (e) {
            showToast('Manual update failed: ' + (e.message || 'error'), 'error');
        }
    });
